...
contract L is ERC721Enumerable, ReentrancyGuard, Ownable {
    bytes public letters = "abcdefghijklmnopqrstuvwxyz";

    // boonked from https://ethereum.stackexchange.com/questions/2519/how-to-convert-a-bytes32-to-string/2834
    function getChar(uint256 index) private view returns (string memory) {
        bytes32 bytedLetter = letters[index];
        uint8 i = 0;
        bytes memory bytesArray = new bytes(32);

        for (i = 0; i < 32; i++) {
            bytesArray[i] = bytedLetter[i];
        }
